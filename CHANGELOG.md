...
### Fixed — a third audit pass, 16 more documentation defects
- **A third independent audit of execution manual v2.3 found sixteen more false claims.** Three audits have now run against this manual and found forty, then six, then sixteen. Two were in repository docs and are fixed here; the rest were in the manual. In all three passes the **dataset itself came through unchanged** — every defect was prose restating a number computed correctly elsewhere, or a claim about a file nobody had opened.
- `docs/reproducibility_tiering.md` said the `score_reproducibility` blind spot affects "both of HYC-0011's rows". HYC-0011 has **four** rows, all four `temperature_unstated`, all four scoring `chahine = 1` for a total of 3 and a suggested Tier C. "Both" also contradicted the same sentence's "the six with a null temperature" (HYC-0011's 4 + HYC-0015's 2).
- `docs/extraction_provenance.md` said "those sixteen HYC-0029 rows" must not enter a Chahine plot. Fifteen are `temperature_cycle`; the sixteenth, HYC-0029-M2, is `isothermal` and characterization-only.
- `scripts/sync_paper_tracking.py`'s `detect_format` docstring said the bad write changed "all 30 lines". The file has **31** physical lines — 30 CRLF terminators plus an unterminated final line — and a CRLF→LF rewrite with an added final newline changes all 31. An off-by-one inside the one docstring whose entire subject is byte-level precision.
- Manual corrections of note, each verified against the artifact: three stale "473 tests" (the suite is at 492); HYC-0029 described as carbon nanofibers when it is `composite` ×12 and `MWCNT` ×4, inside a physics limitation where the material class is the point; the claim that the three-way uptake check "catches the real case" of HYC-0009, which it cannot — all 20 of its rows have only `uptake_wt_pct` populated, so no pair exists and no warning can fire, and a reader is still the only thing that catches that class of defect; `digitize_figure.py --check` written as a flag when `check` is a subcommand; the NUL-byte mechanism attributed to the `csv` module when `append_paper.py` appends raw text (verified end to end: a staging row with a NUL passed all eight preflight checks and exited 0, and `pandas.read_csv` truncates at it on the default C engine but not with `engine="python"`).
- **And the state block had internalised the §6.6 hazard it documents.** It claimed two commits were unpushed. `git ls-remote` showed `origin/main` at `6844f43` while the local ref still read `c99336f`, because `ls-remote` does not update refs — so `git rev-list --count origin/main..HEAD` returned 2 when the true count was 1. Fetch before counting.
